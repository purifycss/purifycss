...
var isRegexWhitelistSelector = function isRegexWhitelistSelector(selector) {
    return selector[0] === "/";
};
        ...
        value: function parseWhitelist(whitelist) {
            ...
            whitelist.forEach(function (whitelistSelector) {
                if (isWildcardWhitelistSelector(whitelistSelector)) {
                    ...
                    _this.patternWhitelist.push(whitelistSelector.substr(1, whitelistSelector.length - 2));
                } else if (isRegexWhitelistSelector(whitelistSelector)) {
                    var regexParser = /^\/(.*)\/(.*)$/;
                    var groups = whitelistSelector.match(regexParser);
                    var regex = new RegExp(groups[1], groups[2]);
                    _this.patternWhitelist.push(regex);
                } else {
